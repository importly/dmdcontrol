...
class DLPC900:
    ...
    def open(self):
        ...
        for intf in range(len(cfg.interfaces())): # pyright: ignore
    ...
    def _command(self, read: bool, cmd_id: int, data: bytes = b""):
        ...
        for off in range(0, max(len(buf), 1), 64):
            ...
            try:
                self.dmd.dev.write(0x01, chunk, timeout=2000) # pyright: ignore
            except usb.core.USBError:
                self.logger.warning('USB write failed, retrying...')
                time.sleep(0.1)
                self.dmd.dev.write(0x01, chunk) # pyright: ignore
        ...
        for _ in range(6):
            try:
                r = bytes(self.dmd.dev.read(0x81, 64, timeout=2000)) # pyright: ignore
            except Exception:

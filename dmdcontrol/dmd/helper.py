...
def select_pyusb_device(usb_bus: int, usb_port: tuple[int, ...]) -> usb.core.Device:
    ...
    matches = [d for d in devices if d.bus == usb_bus and tuple(d.port_numbers or ()) == usb_port]
    if len(matches) != 1:
        seen = ", ".join(f"bus {d.bus} port {list(d.port_numbers or ())}" for d in devices) or "<none>"
        raise RuntimeError(
...
if __name__ == "__main__":
    for d in dlpc900_devices():
        print(f"address {d.address:3d}  ->  usb_bus: {d.bus}   usb_port: {list(d.port_numbers or ())}")

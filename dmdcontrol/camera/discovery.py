...
def configure_rising_edge_triggers(capture):
    ...
    result = {
        "has_setDetectorRisingEdges": callable(
            getattr(capture, "setDetectorRisingEdges", None)
        ),
        "has_setDetectorFallingEdges": callable(
            getattr(capture, "setDetectorFallingEdges", None)
        ),
        "has_setDetectorRunning": callable(
            getattr(capture, "setDetectorRunning", None)
        ),
        "call_order": [],
        "errors": [],
    }
    ...
    for method_name, value in calls:
        ...
        try:
            ...
            result[f"{key}_result"] = (
                ...
                or isinstance(call_result, (bool, int, float, str))
...
def configure_camera_performance(
    capture,
    bias_sensitivity=None,
    efps=None,
    global_hold="default",
):
    result = {
        "requested": {
            "bias_sensitivity": bias_sensitivity or "default",
            "efps": efps or "default",
            "global_hold": global_hold or "default",
        },
        "applied": {
            "contrast_thresholds": False,
            "readout_fps": False,
            "global_hold": False,
        },
    }
    if global_hold in ("on", "off"):
        result["applied"]["global_hold"] = _call_if_available(
            capture, "setGlobalHold", global_hold == "on"
        )
    if bias_sensitivity is not None and bias_sensitivity != "default":
        result["applied"]["contrast_thresholds"] = (
            _configure_dvxplorer_contrast_thresholds(capture, bias_sensitivity)
        )

    if efps is not None and efps != "default":
        result["applied"]["readout_fps"] = _configure_dvxplorer_readout_fps(
            capture, efps
        )
    return result
...
def _configure_dvxplorer_readout_fps(capture, efps):
    ...
    value = (
        getattr(readout_fps, readout_name, None) if readout_fps is not None else None
    )
    if value is None:
        return False

...
def test_filter_rising_triggers_keeps_rising_only():
    triggers = [
        TriggerRecord(timestamp=10, edge="rising"),
        TriggerRecord(timestamp=20, edge="falling"),
        TriggerRecord(timestamp=30, edge="rising"),
    ]
...
def test_linear_accumulation_counts_positive_events_by_trigger_window():
    ...
    events = [
        EventRecord(timestamp=105, x=1, y=2, polarity=True),
        EventRecord(timestamp=110, x=1, y=2, polarity=True),
        EventRecord(timestamp=115, x=3, y=4, polarity=False),
        EventRecord(timestamp=205, x=0, y=0, polarity=True),
    ]
...
def test_linear_accumulation_supports_signed_and_ignore_modes():
    ...
    events = [
        EventRecord(timestamp=100, x=1, y=1, polarity=True),
        EventRecord(timestamp=101, x=1, y=1, polarity=False),
        EventRecord(timestamp=102, x=1, y=1, polarity=False),
    ]
...
def test_events_between_triggers_are_discarded():
    triggers = [
        ...
        TriggerRecord(timestamp=2000, edge="rising"),
    ]
    events = [
        EventRecord(timestamp=1200, x=1, y=1, polarity=True),
        EventRecord(timestamp=1500, x=1, y=1, polarity=True),
        EventRecord(timestamp=1900, x=1, y=1, polarity=True),
    ]
    frames = accumulate_events_for_triggers(
        events, triggers, resolution=(3, 3), window_us=100, polarity_mode="positive"
    )
...
def test_polarity_positive_ignores_negative_events():
    triggers = [TriggerRecord(timestamp=1000, edge="rising")]
    events = [
        EventRecord(timestamp=1010, x=1, y=1, polarity=True),
        EventRecord(timestamp=1020, x=1, y=1, polarity=True),
        EventRecord(timestamp=1030, x=1, y=1, polarity=False),
    ]
    frames = accumulate_events_for_triggers(
        events, triggers, resolution=(3, 3), window_us=100, polarity_mode="positive"
    )
...
def test_polarity_signed_subtracts_negative_events():
    ...
    frames_1 = accumulate_events_for_triggers(
        events_1, triggers, resolution=(3, 3), window_us=100, polarity_mode="signed"
    )
    assert frames_1[0, 1, 1] == 1.0

    events_2 = [
        EventRecord(timestamp=1010, x=1, y=1, polarity=True),
        EventRecord(timestamp=1020, x=1, y=1, polarity=False),
        EventRecord(timestamp=1030, x=1, y=1, polarity=False),
    ]
    frames_2 = accumulate_events_for_triggers(
        events_2, triggers, resolution=(3, 3), window_us=100, polarity_mode="signed"
    )
...
def test_polarity_ignore_counts_all_events_equally():
    ...
    frames = accumulate_events_for_triggers(
        events, triggers, resolution=(3, 3), window_us=100, polarity_mode="ignore"
    )
...
def test_out_of_bounds_events_are_silently_discarded():
    triggers = [TriggerRecord(timestamp=1000, edge="rising")]
    events = [
        EventRecord(timestamp=1010, x=10, y=5, polarity=True),
        EventRecord(timestamp=1020, x=-1, y=5, polarity=True),
        EventRecord(timestamp=1030, x=5, y=10, polarity=True),
        EventRecord(timestamp=1040, x=5, y=-1, polarity=True),
        EventRecord(timestamp=1050, x=5, y=5, polarity=True),
    ]
    frames = accumulate_events_for_triggers(
        events, triggers, resolution=(10, 10), window_us=100, polarity_mode="positive"
    )
...
def test_empty_events_returns_zero_frames():
    triggers = [TriggerRecord(timestamp=1000, edge="rising")]
    frames = accumulate_events_for_triggers(
        [], triggers, resolution=(5, 5), window_us=100, polarity_mode="positive"
    )
...
def test_accumulation_supports_native_numpy_structured_arrays():
    triggers = [TriggerRecord(timestamp=1000, edge="rising")]
    events_arr = np.array(
        [(1000, 1, 1, True), (1050, 1, 1, False)],
        dtype=[("timestamp", np.int64), ("x", np.int16), ("y", np.int16), ("polarity", np.bool_)]
    )
    frames = accumulate_events_for_triggers(
        [events_arr], triggers, resolution=(3, 3), window_us=100, polarity_mode="positive"
    )
    assert frames[0, 1, 1] == 1.0
...
def test_accumulation_sorts_unsorted_events_correctly():
    ...
    # Insert out of order
    events = [
        EventRecord(timestamp=1050, x=1, y=1, polarity=True),
        EventRecord(timestamp=1010, x=1, y=1, polarity=True),
    ]
    frames = accumulate_events_for_triggers(
        events, triggers, resolution=(3, 3), window_us=100, polarity_mode="positive"

...
class PairWrapperTests(unittest.TestCase):
    ...
    def test_run_pair_routes_through_wake_hotplug_and_xinit(self):
        ...
        self.assertIn('dmd_run_xinit_python_module "$SCRIPT_DIR" dmdcontrol pair run -- "$@"', script)
    ...
    def test_pair_only_xinit_client_routes_to_package_cli_after_x11_setup(self):
        ...
        self.assertNotIn("dmd_x11_prepare_single_layout", client)
        self.assertNotIn("--monitor", client)
        self.assertNotIn('case "$LAYOUT"', client)
        self.assertIn(
    ...
    def test_pair_calibration_runner_routes_live_command_through_xinit(self):
        ...
        self.assertLess(wake_idx, xinit_idx)
        self.assertIn('dmd_run_xinit_python_module "$SCRIPT_DIR" dmdcontrol pair calibrate --', script)
    ...
    def test_runners_source_common_shell_helpers(self):
        for name in ("run_dmd_pair.sh", "run_dmd_pair_calibr_square.sh", "run_camera_sync_check.sh", "run_dmd_pair_capture.sh"):
    ...
    def test_single_dmd_launchers_are_removed(self):
        self.assertFalse((ROOT / "run_dmd.sh").exists())
        self.assertFalse((ROOT / "run_calibr_square.sh").exists())
    ...
    def test_common_x11_helper_owns_fixed_60hz_raw_modeline(self):
        ...
        self.assertNotIn("dmd_x11_prepare_single_layout", helper)
    ...
    def test_camera_sync_check_runner_routes_live_capture_through_xinit(self):
        ...
        self.assertIn('dmd_run_xinit_python_module "$SCRIPT_DIR" dmdcontrol camera sync-check -- "$@"', script)
    ...
    def test_pair_capture_runner_routes_live_capture_through_xinit(self):
        ...
        self.assertIn('dmd_run_xinit_python_module "$SCRIPT_DIR" dmdcontrol camera pair-capture -- "$@"', script)
